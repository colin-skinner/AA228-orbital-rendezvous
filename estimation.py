...
class EKF:
    """ 
    ## Attributes:
    - **f** (Function[[np.ndarray, np.ndarray], np.ndarray]): State Transition function
        [ get_f(state, input) --> next_state ]
    - **F** (Function[[np.ndarray], np.ndarray]): State Transition Covariance function
        [ get_F(state, input) --> F ]
    - **h** (Function[[np.ndarray], np.ndarray]): Observation function
        (turns state into what theoretical measurements it would create in that state)
        [ get_h(state) --> meas_theoretical ]
    - **H** (Function[[np.ndarray], np.ndarray]): Observation Covariance function
        [ get_H(state) --> H ]
    - **Q** (np.ndarray): Process Noise Covariance. Typically a diagonal matrix where each elements
        the (expected) covariance of each element of the state
    - **R** (np.ndarray): Measurement Noise Covariance. Typically a diagonal matrix where each elements
...
def run_kf(state: State, ekf: KalmanFilter, u: np.ndarray, z: np.ndarray):
    ...
    P_next = (np.eye(dim) - K@H) @ P_est @ (np.eye(dim) - K@H).T + K@R@K.T # Joseph form
    return State(x_next, P_next)

def run_ekf(state: State, ekf: EKF, u: np.ndarray, z: np.ndarray):
    f, get_F, h, get_H, Q, R = ekf.f, ekf.F, ekf.h, ekf.H, ekf.Q, ekf.R
    x, P = state.x, state.P
    dim = len(state.x)
